def leepYearCheck(year:int)->bool:
    """
    檢查是否是閏年
...
def monthDaysCheck(year:int, month:int) ->int:
    """
    檢查月份的總天數
...
def monthDayTotal(year:int, month:int, day:int)->int:
    """
...
def diffYearDays(ya:int, ma:int, da:int, yb:int, mb:int, db:int)->int:
    """
    統計天數差距
    其中 ya < yb
