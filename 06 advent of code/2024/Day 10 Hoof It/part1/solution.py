...
class Solution:
    ...
    def calculate_score(self, row: int, col: int) -> int:
        # bfs
        queue: deque[Point] = deque([Point(row, col)])
        visited: set[Point] = {Point(row, col)}
        while queue:
            point = queue.popleft()
            high: int = self.graph[point.row][point.col]
            for pd in Direction:
                npoint: Point = point + pd.value
                if not self._is_valid_point(npoint):
                    continue
                if npoint in visited:
                    continue
                nhigh: int = self.graph[npoint.row][npoint.col]
                if nhigh - high != 1:
                    continue
                queue.append(npoint)
                visited.add(npoint)
        return sum(self.graph[p.row][p.col] == 9 for p in visited)
