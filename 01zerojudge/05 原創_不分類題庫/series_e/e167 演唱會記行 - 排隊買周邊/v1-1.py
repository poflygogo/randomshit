...
direction_side = [(i, j) for i in range(-1, 2) for j in range(-1, 2)]
direction_wide = [(i, j) for i in range(-2, 3) for j in range(-2, 3)]
direction_side.remove((0, 0))
direction_wide.remove((0, 0))
seen = set()
...
# dfs
def _find_the_tail(max_row: int, max_col: int, graph: list, row: int, col: int, jump: bool = True):
    """
    args:
        max_row(int): 最大高度
        max_col(int): 最大寬度
