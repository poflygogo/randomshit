...
while len(arr) > 1:
    ...
    for i in range(1, len(arr), 2):
        ...
        if mul_a >= mul_b:
            ...
            data[arr[i]][1] += data[arr[i]][1] // 2
            data[arr[i]][2] += 1
            win.append(arr[i - 1])
            if data[arr[i]][2] < m:
                loss.append(arr[i])
        else:
            data[arr[i]][0] += mul_a // (2 * data[arr[i]][1])
            data[arr[i]][1] += mul_a // (2 * data[arr[i]][0])
            data[arr[i - 1]][0] += data[arr[i - 1]][0] // 2
            data[arr[i - 1]][1] += data[arr[i - 1]][1] // 2
            data[arr[i - 1]][2] += 1
            win.append(arr[i])
            if data[arr[i - 1]][2] < m:
                loss.append(arr[i - 1])
