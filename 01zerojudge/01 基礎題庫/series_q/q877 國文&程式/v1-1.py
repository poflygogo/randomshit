...
if a_depth < b_depth:
    ...
    print(sorted([a, b], key=lambda x: age_order[graph[a]].index(x))[0])
elif graph[graph[a]] == graph[graph[b]]:
    print(sorted([a, b], key=lambda x: age_order[graph[graph[x]]].index(graph[x]))[0])
