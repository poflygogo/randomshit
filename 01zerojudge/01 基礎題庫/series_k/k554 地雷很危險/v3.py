...
num_row, num_column = map(int, stdin.readline().split())
danger = [[0 for _ in range(num_column)] for _ in range(num_row)]

for row in range(num_row):
    for column, item in enumerate(map(int, stdin.readline().split())):
        if item != 0:
            if item == 1:
...
result = '\n'.join(' '.join(str(i) for i in sublist) for sublist in danger)
stdout.write(result)