...
barrier = {tuple(map(int, input().split())) for _ in range(p)}  # 障礙物
no_left = {tuple(map(int, input().split())) for _ in range(q)}  # 禁止左轉的格子
...
while queue:
    ...
    if (x, y) in no_left:
        iterator = ((d + i) % 4 for i in range(2))      # 直走、右轉
    else:
        iterator = ((d + i) % 4 for i in range(-1, 2))  # 左轉、直走、右轉
    ...
    for i in iterator:
        ...
        if (0 <= nx < n and 
            ...
            seen.get((nx, ny), {}).get(i, float('inf')) > step):
            seen[(nx, ny)][i] = seen.get((nx, ny), {i: step}).get(i, step)
            queue.append((nx, ny, i, step))

print(min(seen.get((tx, ty), {None: -1}).values()))
